...
public class SimpleBankingApp {
	...
	public static void printAllUsers() {
	    ...
	    for (User user : users) {
	        System.out.println(user.toFormattedString());
	    }
	...
	public static void printAllAccounts() {
	    ...
	    System.out.println(String.format("%-10s| %-30s| %-10s| %-22s| %-15s",
	            "Account #", "username_of_account_holder", "type", "opening_date", "Balance"));
	    System.out.println("--------------------------------------------------------------------------------");

	    for (Account account : accounts) {
	        String accountNumber = account.getAccount_number();
	        String username = account.getUsername_of_account_holder();
	        String type = account.getAccount_type();
	        String openingDate = new SimpleDateFormat("MMM dd, yyyy").format(account.getAccount_opening_date());
	        double balance = getBalance(accountNumber);

	        System.out.println(String.format("%-10s| %-30s| %-10s| %-22s| $%.2f",
	                accountNumber, username, type, openingDate, balance));
	    }
	}
